...
#[post("/search", data="<keywords>")]
pub async fn search(keywords: String, _key: JWT) -> Result<Json<ReqwestStatus>, String> {
    let url = format!("https://api.upcitemdb.com/prod/trial/search?s={}", keywords);
    let response = reqwest::get(&url).await
        .map_err(|err|format!("Error getting url content : {}", err))?
        .json::<serde_json::Value>()
        .await
